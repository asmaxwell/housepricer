...
#create marker to skip heavy tests unless specifically requested
#Taken from: https://stackoverflow.com/questions/56374588/how-can-i-ensure-tests-with-a-marker-are-only-run-if-explicitly-asked-in-pytest
def pytest_collection_modifyitems(config, items):
    ...
    skip_heavy_test = pytest.mark.skip(reason='heavy_tests not selected')
    for item in items:
        if 'heavy_tests' in item.keywords:
            item.add_marker(skip_heavy_test)
...
@pytest.mark.heavy_tests
def test_random_hyperparameter_search(load_database) -> None:
    rf = load_database
    rf.random_hyperparameter_search(10)
    assert(rf.model.get_params()["randomforestregressor__n_estimators"] != 100)

@pytest.mark.heavy_tests
def test_evolve_hyperparameter_search(load_database) -> None:
    rf = load_database
    rf.evolve_hyperparameter_search(10, 5)
    assert(rf.model.get_params()["randomforestregressor__n_estimators"] != 100)